...
class Book {
  ...
  static async partialUpdate(isbn, updateData) {
    // Example logic for partial update
    const fields = Object.keys(updateData);
    const setters = fields.map((field, index) => `${field}=$${index + 1}`);
    const query = `UPDATE books SET ${setters.join(", ")} WHERE isbn=$${fields.length + 1} RETURNING *`;
  
    const values = [...Object.values(updateData), isbn];
